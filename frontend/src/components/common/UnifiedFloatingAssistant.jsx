...
const UnifiedFloatingAssistant = () => {
  ...
  const [widgetPosition, setWidgetPosition] = useState('top');
    ...
    if (activeWidget === type) {
      ...
      if (constraintsRef.current) {
        ...
        if (dock) {
          ...
          setWidgetPosition(rect.top < 400 ? 'bottom' : 'top');
        }
      }
      setActiveWidget(type);
      setIsMinimized(false);
      setIsDockCollapsed(false); // Auto expand if opening a widget
    }
  };
      ...
      >
        ...
        <AnimatePresence>
          {activeWidget && !isMinimized && (
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: widgetPosition === 'top' ? 20 : -20, filter: 'blur(10px)' }}
              animate={{ opacity: 1, scale: 1, y: 0, filter: 'blur(0px)' }}
              exit={{ opacity: 0, scale: 0.9, y: widgetPosition === 'top' ? 20 : -20, filter: 'blur(10px)' }}
              className={`relative w-[400px] max-h-[70vh] flex flex-col ${widgetPosition === 'top' ? 'mb-6 order-first' : 'mt-6 order-last'}`}
            >
        ...
        <motion.div 
          layout
          id="unified-dock"
          className={`flex items-center gap-2 bg-[#0F172A]/90 backdrop-blur-2xl p-2 rounded-[24px] shadow-2xl border border-white/10 transition-all duration-500 ${isDockCollapsed ? 'w-14 h-14' : 'px-2.5'}`}
        >
          {/* COLLAPSE/EXPAND TOGGLE (ONLY IN COLLAPSED MODE) */}
          {isDockCollapsed ? (
            <motion.button
              layoutId="dock-icon"
              onClick={() => setIsDockCollapsed(false)}
              className="w-10 h-10 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg animate-pulse"
            >
               <Activity size={20} />
            </motion.button>
          ) : (
            <>
              ...
              {/* QUICK STATUS / MINIMIZE DOCK */}
              <button 
                onClick={() => setIsDockCollapsed(true)}
                className="flex items-center gap-4 pl-3 pr-5 hover:bg-white/5 rounded-2xl transition-all h-14 group"
              >
                 <div className="flex flex-col items-end">
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1 group-hover:text-blue-400">US-EAST-1</span>
