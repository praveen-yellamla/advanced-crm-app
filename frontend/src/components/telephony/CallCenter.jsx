...
const CallCenter = ({ embedded = false }) => {
  const { 
    callState, 
    isMuted, 
    duration, 
  ...
  const dialerContent = (
    <div className={`flex flex-col bg-white overflow-hidden ${embedded ? 'w-[380px]' : ''}`}>
      {/* PREMIUM HEADER */}
         ...
         <AnimatePresence mode="wait">
            {callState === 'idle' ? (
              <motion.div key="dialer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-10">
                 ...
                 <div className="grid grid-cols-3 gap-6 max-w-[280px] mx-auto">
                    {[1,2,3,4,5,6,7,8,9,'*',0,'#'].map((num) => (
                      <button 
                        ...
                        className="aspect-square rounded-[28px] bg-slate-50 hover:bg-slate-100 text-slate-800 font-black text-2xl active:scale-90 transition-all flex flex-col items-center justify-center group border border-slate-100 shadow-sm"
                      >
                 ...
                 <button 
                   onClick={handleCall}
                   className="w-full h-20 bg-blue-600 text-white rounded-[28px] font-black uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/40 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-4 mt-6"
                 >
              ...
              <motion.div key="active" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12 py-4">
                 ...
                 <div className="grid grid-cols-1 gap-8">
                    ...
                    {showKeypad && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="grid grid-cols-3 gap-4 pt-8 border-t border-slate-100 max-w-[240px] mx-auto">
                         {[1,2,3,4,5,6,7,8,9,'*',0,'#'].map((num) => (
                           <button 
                             key={num}
                             onClick={() => handleDial(num)}
                             className="aspect-square rounded-2xl bg-slate-50 hover:bg-slate-100 text-slate-500 font-black text-xl active:scale-90 transition-all border border-slate-100 flex items-center justify-center"
                           >
                              {num}
                           </button>
                         ))}
  ...
  if (embedded) return dialerContent;
